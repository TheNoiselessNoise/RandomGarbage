...
namespace UniversalTrainer
{
    ...
    public class KeyStruct
    {
        ...
        public bool NotPrefix { get; set; } // only for KeyType.UN_FREEZE

        public KeyStruct(int id, int codeid, KeyType type, KeyCode code, string prefix = null, string suffix = null, int? value = null, string helptip = null, bool notprefix = false)
        {
            Id = id;
            CodeId = codeid;
            Type = type;
    ...
    public class TrainerConfig
    {
        ...
        public static TrainerLanguage lang = new TrainerLanguage();
        ...
        public static void DeactivateKeys(bool playsound = true)
        {
            bool _speaker = IsSpeaker;
        ...
        {
            aks.Text = lang.ACTIVATE_TRAINER;
        ...
        public static void SetLanguage(TrainerLanguage language)
        {
            lang = language;
        }
        ...
        public static SimpleTextStruct GetSimpleText(KeyStruct ks, CodeStruct cs = null, Font font = null, bool withkeys = true)
        {
            ...
            if (ks.Type == KeyType.ADD || ks.Type == KeyType.SET)
            {
                ...
                {
                    prefix = lang.ADD_INPUT_TEXT;

                    if (ks.Type == KeyType.SET)
                    {
                        prefix = lang.SET_INPUT_TEXT;
                    }
                }
            ...
            else if (ks.Type == KeyType.UN_FREEZE)
            {
                if (ks.NotPrefix)
                {
                    return new SimpleTextStruct()
                    {
                        Prefix = withkeys ? $"({ks.Code.ToString()}) {ks.Prefix} {cs.Name} {ks.Suffix}" : $"{ks.Prefix} {cs.Name} {ks.Suffix}"
                    };
                }
                return new SimpleTextStruct()
                {
                    Prefix = withkeys ? $"({ks.Code.ToString()}) Un/Freeze {ks.Prefix} {cs.Name} {ks.Suffix}" : $"Un/Freeze {ks.Prefix} {cs.Name} {ks.Suffix}"
                };
            }
            ...
            try
            {
                ...
                else if (ks.Type == KeyType.UN_FREEZE)
                {
                    ...
                    if (cs.IsFrozen)
                    {
                        ...
                        {
                            value = TrainerConfig.GetValue(cs);
                        }
